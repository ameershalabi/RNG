...
def hex_to_signed(hex_str, bits=512):
...
async def mytest150(dut):
    ...
    dut.init_i.value = 1
    dut.gen_i.value = 1
    SEED = 0x285F44AB321ED5E5D7F04BB5AD4D5B981EC6F4E08D475D5F8E7DBBB955F6389DFC7FFDD5E74EF4C24F7F1722EF758AC4DB33FA5E3C4D8A2C36BD0F75A4ABE07D
    RULES = 0xFC7FFDD5E74EF4C24F7F1722EF758AC4DB33FA5E3C4D8A2C36BD0F75A4ABE07D285F44AB321ED5E5D7F04BB5AD4D5B981EC6F4E08D475D5F8E7DBBB955F6389D
    dut.seed_i.value = SEED
    dut.rule_i.value = RULES
    ...
    signed_value = SEED & ((1 << 512) - 1)
    signed_rules = RULES & ((1 << 512) - 1)
    SEED = [int(b) for b in format(signed_value, '0512b')][::-1]
    RULES = [int(b) for b in format(signed_rules, '0512b')][::-1]
    model = casr_90150h(SEED, RULES, mode='p')

    model_outputs = prep_out(model.generate(10),512)
